...
describe("CharacterStage", () => {
  ...
  it("applies the softened cast treatment when requested", () => {
    render(
      <CharacterStage
        stage={{
          mode: "duo-stage",
    ...
    expect(screen.getByTestId("portrait-jane").parentElement).toHaveClass("opacity-65");
    expect(screen.getByTestId("portrait-rochester").parentElement).toHaveClass("opacity-65");
  });
});
